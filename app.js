document.addEventListener("DOMContentLoaded", async () => {
  ...
  const timeElapse = (date) => {
    ...
    const days = Math.floor(seconds / (3600 * 24)) + 2;
    seconds %= 3600 * 24;
    ...
    // const result = `<span class="digit">${days}</span> ngày <span class="digit">${hours}</span> tháng <span class="digit">${minutes}</span> năm`;
